...
def normalize_data(df, maxv, minv):
    """
        Normalizes the df given the maximum and minimum values of each variable.
        Parameters
        ----------
        df: numpy n x m
            Numpy array comprising n realisations (instances) of m variables.
...
def standardize_data(df, mean, std):
    """
        Standardizes the df given the mean and standard deviations values of
        each variable.
        Parameters
        ----------
        df: numpy n x m
...
X_train, X_test, y_train, y_test, data_train, data_test = gefs_train_test_split(df, ncat)
...
def gefs_train_test_split(df, ncat, train_ratio=0.7, prep='std'):
    assert train_ratio >= 0
    assert train_ratio <= 1
